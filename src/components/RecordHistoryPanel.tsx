// src/components/RecordHistoryPanel.tsx
"use client";
...
export function RecordHistoryPanel({ mode }: PanelProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const defaultCats =
        mode === "business"
          ? defaultBusinessCategories
          : defaultPersonalCategories;

      if (categoriesSnap.empty) {
        for (const type of ["expense", "income"] as const) {
          for (const cat of defaultCats[type]) {
            const newCatRef = doc(categoriesRef, cat);
            await setDoc(newCatRef, { name: cat, type });
          }
        }
        setCategories([...defaultCats.expense, ...defaultCats.income]);
      } else {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">記帳紀錄</h1>
